...
function normalizeFilePath(filePath) {
  ...
  if (projectIndex !== -1) {
    ...
    if (
      (parts[projectIndex] === "workspace" || parts[projectIndex] === "work") &&
...
Object.entries(data).forEach(([file, stats]) => {
  ...
  console.log(
    `| ${emoji} ${normalizedPath} | ${totalLines} stmts (${totalFuncs} funcs) | ${coveredLines} (${coveredFuncs}) | ${pct}% (funcs: ${funcPct}%) |`,
  );
});
