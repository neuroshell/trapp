...
export class LogPage extends BasePage {
  ...
  constructor(page: Page) {
    super(page);
    this.logScreen = page.getByText('Log Workout', { exact: false }).or(
      page.getByTestId('log-screen')
    );
    
    // Workout type selectors
    this.workoutTypeSelector = page.getByTestId('workout-type-selector').or(
      page.getByRole('tabgroup')
    );
    this.runningTypeButton = page.getByTestId('type-running').or(
      page.getByText('Running', { exact: false }).first()
    );
    this.squatsTypeButton = page.getByTestId('type-squats').or(
      page.getByText('Squats', { exact: false })
    );
    this.pushupsTypeButton = page.getByTestId('type-pushups').or(
      page.getByText('Pushups', { exact: false })
    );
    this.pullupsTypeButton = page.getByTestId('type-pullups').or(
      page.getByText('Pullups', { exact: false })
    );
    
    // Running fields
    this.distanceInput = page.getByTestId('distance-input').or(
      page.getByLabel('Distance', { exact: false }).or(
        page.getByPlaceholder(/distance/i)
      )
    );
    this.durationInput = page.getByTestId('duration-input').or(
      page.getByLabel('Duration', { exact: false }).or(
        page.getByPlaceholder(/duration/i)
      )
    );
    this.distanceError = page.getByTestId('distance-error').or(
      page.getByText(/valid distance/i)
    );
    this.durationError = page.getByTestId('duration-error').or(
      page.getByText(/valid duration/i)
    );
    
    // Strength fields
    this.repsInput = page.getByTestId('reps-input').or(
      page.getByLabel('Reps', { exact: false }).or(
        page.getByPlaceholder(/reps/i)
      )
    );
    this.setsInput = page.getByTestId('sets-input').or(
      page.getByLabel('Sets', { exact: false }).or(
        page.getByPlaceholder(/sets/i)
      )
    );
    this.weightInput = page.getByTestId('weight-input').or(
      page.getByLabel('Weight', { exact: false }).or(
        page.getByPlaceholder(/weight/i)
      )
    );
    this.repsError = page.getByTestId('reps-error').or(
      page.getByText(/valid reps/i)
    );
    this.setsError = page.getByTestId('sets-error').or(
      page.getByText(/valid sets/i)
    );
    this.weightError = page.getByTestId('weight-error').or(
      page.getByText(/valid weight/i)
    );
    
    // Common fields
    this.notesInput = page.getByTestId('notes-input').or(
      page.getByLabel('Notes', { exact: false }).or(
        page.getByPlaceholder(/notes/i)
      )
    );
    this.dateTimePicker = page.getByTestId('datetime-picker').or(
      page.getByText('Date', { exact: false }).first()
    );
    this.saveButton = page.getByTestId('save-workout-button').or(
      page.getByText('Save', { exact: false }).or(
        page.getByText('Log Workout', { exact: false }).last()
      )
    );
    this.cancelButton = page.getByTestId('cancel-button').or(
      page.getByText('Cancel', { exact: false })
    );
    
    // Delete functionality
    this.deleteButton = page.getByTestId('delete-workout-button').or(
      page.getByText('Delete', { exact: false }).first()
    );
    this.deleteConfirmationDialog = page.getByTestId('delete-confirmation-dialog').or(
      page.getByText(/are you sure/i).or(page.getByText(/cannot be undone/i))
    );
    this.confirmDeleteButton = page.getByTestId('delete-button').or(
      page.getByText('Delete', { exact: false }).last()
    );
    this.cancelDeleteButton = page.getByTestId('cancel-button').or(
      page.getByText('Cancel', { exact: false }).last()
    );
    
    // Workout list
    this.workoutList = page.getByTestId('workout-list').or(
      page.getByRole('list')
    );
    ...
    // Messages
    this.successToast = page.getByText(/workout logged/i).or(
  ...
  async goto() {
    ...
    // Navigate to log tab
    const logTab = this.page.getByTestId('tab-log').or(
      this.page.getByRole('tab', { name: /log/i })
    );
    await logTab.click();
  ...
  async selectWorkoutType(type: 'running' | 'squats' | 'pushups' | 'pullups') {
    const button = this.page.getByTestId(`type-${type}`).or(
      this.page.getByText(type.charAt(0).toUpperCase() + type.slice(1), { exact: false })
    );
    await button.click();
  }
   ...
   */
  async fillStrengthWorkout(reps: string, sets: string, weight?: string, notes?: string) {
   ...
   */
  async expectWorkoutTypeSelected(type: 'running' | 'squats' | 'pushups' | 'pullups') {
    const button = this.page.getByTestId(`type-${type}`);
    await expect(button).toHaveAttribute('aria-selected', 'true').or(
      expect(button).toHaveCSS('background-color', /rgb/)
    );
  }
  ...
  async deleteWorkout() {
    ...
    await this.deleteConfirmationDialog.waitFor({ state: 'visible', timeout: 5000 });
   ...
   */
  async expectFormPreFilled(distance?: string, duration?: string, reps?: string, sets?: string) {
    if (distance) {
