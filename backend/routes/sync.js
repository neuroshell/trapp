    ...
    if (since) {
      ...
      filteredData = {
        workouts: userData.workouts.filter(
          (w) => new Date(w.updatedAt) >= sinceDate,
        ),
        achievements: userData.achievements.filter(
          (a) => new Date(a.unlockedAt) >= sinceDate,
        ),
      ...
      if (filteredData.workouts.length > 0) {
        logger.info(
          `Filtered workouts: ${JSON.stringify(filteredData.workouts.map((w) => ({ id: w.id, updatedAt: w.updatedAt })))}`,
        );
      }
    }

    logger.info(
      `Sync download complete for user ${userId}: ${filteredData.workouts.length} workouts`,
    );
...
router.post("/", authenticate, validateSyncRequest, async (req, res, next) => {
  try {
    ...
    logger.info(
      `Sync upload requested for user ${userId}: ${workouts?.length || 0} workouts`,
    );
    ...
    logger.info(
      `Sync upload complete for user ${userId}: ${result.conflicts} conflicts resolved`,
    );
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      logger.info(
        `Workout saved successfully: ${savedWorkout.id} for user ${userId}`,
      );
