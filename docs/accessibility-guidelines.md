...
function AccessibleText({ children, size = 'medium' }) {
  ...
  return (
    <Text style={{ fontSize: fontSizes[size] }}>
      {children}
    </Text>
  );
}
...
*This accessibility documentation should be referenced throughout development. Accessibility is not a feature—it's a fundamental requirement.*
